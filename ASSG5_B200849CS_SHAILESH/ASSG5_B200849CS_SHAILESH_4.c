...
int coundNodes(struct Node **List)
{
    int count = 0;
...
void PRINT_REVERSE(Node **List, int x, int y)
{
    if (*List == NULL)
    {
        // list is empty
    ...
    {
        int numberOfNodes = coundNodes(List);

        if (x > numberOfNodes || y > numberOfNodes)
        {
            // if x or y is greater than number of nodes in list
            printf("NULL\n");
            return;
        }
        else
        {
            // the condition x < numberOfNodes && y < numberOfNodes
            struct Node *begin = *List;
            for (int i = 0; i < x - 1; i++)
            {
                begin = begin->next;
            }
